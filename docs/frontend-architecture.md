...
- 报告重建沿用现有工作区卡片：`report_job` presentation 只携带任务 ID，浏览器通过独立任务服务读取
  实际进度，每 2 秒查询，连接失败后每 5 秒重连，终态停止轮询。无状态变化时不重绘，更新时保留
  失败详情的展开状态与操作按钮焦点。
- 任务卡片支持取消、失败子集重试和移除已结束卡片。重试请求 ID 在操作前保存，网络响应丢失或刷新
  页面后重复操作仍复用同一次提交。浏览器只保存任务标识及重试所需标识，不保存报告或进度副本。
- 收到报告任务时自动展开工作区；刷新后重新查询已跟踪任务。后续提问、清除对话上下文不移除任务卡片，
  也不取消后台执行。窄屏有报告任务时使用上下排列的对话区和结果区；取消不依赖模型可用性。
